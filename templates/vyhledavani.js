...
function vyhledavani(event){
	...
	for (const zvire in zvirata) {
			let foo = zvirata[zvire];

			var nazev = foo.c;
			var zviresmezerou= zvire.replace(spojka, ' ');
			var zviresmezeroupozpatku = zviresmezerou.split(' ').reverse().join(' ');
			var dokument = zvire.replace(spojka, '-')+'.html';
			var vnazvu = zviresmezerou.match(re);
			var vnazvupozpatku = zviresmezeroupozpatku.match(re);
